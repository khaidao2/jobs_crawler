...
def _load_schema_defaults(schema_path: str) -> tuple:
    """Load Avro schema to auto-fill missing fields (Schema-Driven Mapping).
...
class GenericCrawler:
    def __init__(self, config_path: str, sink: KafkaSink, schema_path: str,
        ...
        self.sink = sink
        #Load schema-driven defaults once at startup
    ...
    async def fetch_page(self, session: AsyncSession, url: str) -> str:
        """Retry with exponential backoff. Rate-limited via semaphore.
        ...
        for attempt in range(3):
            try:
                async with self.semaphore:
                    ...
                    #Rotate User-Agent per request to minimize blocking
    ...
    def parse_item(self, container: BeautifulSoup) -> Dict[str, Any]:
        ...
        # Start with schema defaults (Load schema-driven defaults)
        data = dict(self.schema_defaults)
        
        # Add metadata (Add crawled timestamp)
        ...
        # Post-processing: Generate stable job_id from SHA256 of URL
        if data.get('url'):
        ...
        # Post-processing: Parse salary and experience
